...
import { listUserCourseAll, addUserCourse, delUserCourse, getUserCourse,findByStudentId } from '../api/user_course';
...
if(route.query.id){
  findByStudentId(route.query.id).then(res=>{
  userList.value = res.data
});
}
...
function getRecord(id) {
  ...
  getUserCourse(id).then(res => {
    if (res.code == '200') {
      value.value = res.data.map(item => item.c_id - 0)
