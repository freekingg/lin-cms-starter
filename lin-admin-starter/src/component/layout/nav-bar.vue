...
export default {
  ...
  data() {
    return {
      ...
      hidden: false,
      messages: [],
      path: `//localhost:15001/ws/message?token=${getToken('access_token').split(' ')[1]}`,
    }
  },
  created() {
    if (Config.websocketEnable) {
      this.$connect(this.path, { format: 'json', reconnection: true })
      ...
      this.$options.sockets.onerror = err => {
        console.error(err)
        this.$message.error('socket-token已过期,请重新登录')
        setTimeout(() => {
